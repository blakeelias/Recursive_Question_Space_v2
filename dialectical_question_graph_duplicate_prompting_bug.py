...
class DialecticalGraph:
    ...
    def __init__(self, api_key: str, central_question: str, nonsense_threshold = 95, view_identity_threshold = 95, prompt_dir: str = "./prompts", 
        ...
        # If we loaded from a save, try to restore FAISS index
        if resuming_from_save:
            try:
    ...
    def initialize_graph(self) -> None:
        ...
        # Step 1: Generate initial thesis nodes if they don't exist yet
        ...
        for thesis_id in thesis_nodes:
            if thesis_id not in self.processed_theses:
                self._generate_reasons(thesis_id)
                self.processed_theses.add(thesis_id)
                self._save_state()
